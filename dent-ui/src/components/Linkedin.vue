      ...
      <v-flex md12>
        <v-card>
          ...
          <v-dialog v-model="dialog" max-width="500px">
            ...
            <v-card>
              <v-card-title>
                <span v-if="editedItem.status">
                  Edit {{ editedItem.email }}
                </span>
                <span v-else>Create</span>
              </v-card-title>
              <v-card-text>
                <v-row>
                  <v-col v-if="editedItem.status" cols="12" sm="3">
                    <v-text-field
...
export default {
  ...
  mounted() {
    this.writeAccess = routeUtil
      .checkUserComponentAccess("Linkedin")
      .includes("w");
  ...
  methods: {
    ...
    changeStatus(item) {
      call
        .api({
          endpoint: "/management/linkedin/" + item.email,
          body: item
        })
        .then(response => {
          console.log(JSON.stringify(response));
    ...
    deleteItem(item) {
      call
        .api({
          endpoint: "/management/linkedin/delete/" + item.email,
          method: "GET"
        })
        .then(response => {
          console.log(JSON.stringify(response));
